...
import { NotificationContext } from "../context/NotificationContext"; // Importa il contesto

import SearchBar from "../components/SearchBar";
import "../style/Header.css";

export default function Header() {
    // Prendi il numero totale di articoli nel carrello dal context
    const { cartCount } = useContext(NotificationContext); // Consuma cartCount dal contesto
            ...
            <header>
                <div className='container_header'>
                    ...
                    <section className='icon-header'>
                        ...
                        {/* Icona del carrello con numeretto */}
                        <Link to={"/cart"} onClick={() => window.scrollTo(0, 0)}>
                            <FontAwesomeIcon icon={faBasketShopping} style={{ color: "#000000" }} />
                            {cartCount > 0 && <span className="cart-count">{cartCount}</span>} {/* Mostra numeretto se il carrello non è vuoto */}
                        </Link>
